...
聚焦 **WireGuard 组网 + 多链路 + 负载均衡**。v1.2 全面重构了前端界面（新设计系统、深浅色主题、实时仪表盘），并新增转发批量操作、克隆、导入导出等高效管理能力。
...
- 实时显示接口、监听端口、握手时间、Endpoint、AllowedIPs 与收发流量；拓扑示意直观展示 Mesh / Hub 结构
...
### 3. 转发高效管理（v1.2 新增）
- **表格化转发列表**：状态/入口/目标/策略/延迟/流量一屏总览，支持关键词搜索与状态筛选
- **批量操作**：多选后批量暂停 / 恢复 / 删除
- **一键克隆**：复用现有转发的组与目标，自动分配新端口
- **导入 / 导出**：全部转发一键导出为 JSON，迁移或备份后可按原拓扑重建（复用一体化创建，失败自动回滚）
- **单条重新下发**：节点重启或配置漂移后无需删除重建，保持暂停态
- 排序支持上移/下移，链路诊断结果直接展示每跳延迟与丢包
...
### 7. 全新界面（v1.2）
- 前端彻底重写：移除重型组件库，基于 Tailwind 4 的自研设计系统，构建产物更小、加载更快
- 深色 / 浅色主题一键切换（记住偏好，默认跟随系统）
- 实时仪表盘：WebSocket 推送的节点在线状态与延迟、流量 Top 榜、24 小时流量趋势、快捷入口
- 完整移动端适配：H5 底部导航、安全区适配，功能与桌面端一致
...
```
vite-frontend (React + Tailwind 4)  ──►  springboot-backend (Spring Boot, :6365)
...
```
面板中"节点监控 → 新增节点"会直接给出完整命令，复制执行即可。依赖（wireguard-tools / iproute2）自动安装。
...
4. **日常运维**：转发列表支持批量暂停/恢复、克隆、导入导出与单条重下发
5. **高级维护（可选）**：在线路管理或路由组中复用、调整系统自动生成的资源
...
v1.2 未新增数据库表和字段，旧版数据直接兼容，升级面板不会影响已有转发配置。
...
3. 创建 tag `1.2.0` 与 Release，上传安装脚本 / docker-compose / panel.sql

> 从 1.0.x 升级：v1.1 新增 `WgPrepare` 与 `WgStatus` 节点命令。旧节点仍可兼容同步；1.1.3 起可直接在节点页通过受信控制通道完成跨版本升级，无需先登录节点 SSH。
>
> v1.2 面板与 Agent 完全向下兼容：升级面板后 1.1.3 Agent 无需立即升级；新功能（批量操作、克隆、导入导出、重新下发）对旧 Agent 同样可用。
