package selector

import (
	"context"
	"hash/crc32"
	"math/rand"
	"strings"
	"sync"
	"sync/atomic"
	"time"

	"github.com/go-gost/core/logger"
	"github.com/go-gost/core/metadata"
	mdutil "github.com/go-gost/x/metadata/util"
	"github.com/go-gost/core/selector"
	ctxvalue "github.com/go-gost/x/ctx"
)

type roundRobinStrategy[T any] struct {
	counter uint64
}

// RoundRobinStrategy is a strategy for node selector.
// The node will be selected by round-robin algorithm.
func RoundRobinStrategy[T any]() selector.Strategy[T] {
	return &roundRobinStrategy[T]{}
}

func (s *roundRobinStrategy[T]) Apply(ctx context.Context, vs ...T) (v T) {
	if len(vs) == 0 {
		return
	}

	n := atomic.AddUint64(&s.counter, 1) - 1
	return vs[int(n%uint64(len(vs)))]
}

type randomStrategy[T any] struct {
	rw *RandomWeighted[T]
	mu sync.Mutex
}

// RandomStrategy is a strategy for node selector.
// The node will be selected randomly.
func RandomStrategy[T any]() selector.Strategy[T] {
	return &randomStrategy[T]{
		rw: NewRandomWeighted[T](),
	}
}

func (s *randomStrategy[T]) Apply(ctx context.Context, vs ...T) (v T) {
	if len(vs) == 0 {
		return
	}

	s.mu.Lock()
	defer s.mu.Unlock()

	s.rw.Reset()
	for i := range vs {
		weight := 0
		if md, _ := any(vs[i]).(metadata.Metadatable); md != nil {
			weight = mdutil.GetInt(md.Metadata(), labelWeight)
		}
		if weight <= 0 {
			weight = 1
		}
		s.rw.Add(vs[i], weight)
	}

	return s.rw.Next()
}

type fifoStrategy[T any] struct{}

// FIFOStrategy is a strategy for node selector.
// The node will be selected from first to last,
// and will stick to the selected node until it is failed.
func FIFOStrategy[T any]() selector.Strategy[T] {
	return &fifoStrategy[T]{}
}

// Apply applies the fifo strategy for the nodes.
func (s *fifoStrategy[T]) Apply(ctx context.Context, vs ...T) (v T) {
	if len(vs) == 0 {
		return
	}
	return vs[0]
}

type hashStrategy[T any] struct {
	r  *rand.Rand
	mu sync.Mutex
}

func HashStrategy[T any]() selector.Strategy[T] {
	return &hashStrategy[T]{
		r: rand.New(rand.NewSource(time.Now().UnixNano())),
	}
}

func (s *hashStrategy[T]) Apply(ctx context.Context, vs ...T) (v T) {
	if len(vs) == 0 {
		return
	}
	if h := ctxvalue.HashFromContext(ctx); h != nil {
		value := uint64(crc32.ChecksumIEEE([]byte(h.Source)))
		logger.Default().Tracef("hash %s %d", h.Source, value)
		return vs[value%uint64(len(vs))]
	}

	s.mu.Lock()
	defer s.mu.Unlock()

	return vs[s.r.Intn(len(vs))]
}

// LatencyProvider 延迟数据源接口, 由运行环境(节点代理)注入
type LatencyProvider interface {
	// GetLatency 返回指定地址最近延迟(毫秒), 不存在或不可达返回 false
	GetLatency(addr string) (ms int64, ok bool)
}

var latencyProvider LatencyProvider

// SetLatencyProvider 注册延迟数据源
func SetLatencyProvider(p LatencyProvider) {
	latencyProvider = p
}

// probeAddrOf 提取候选对象的探测地址
func probeAddrOf(v any) (string, bool) {
	if md, ok := v.(metadata.Metadatable); ok && md != nil {
		if addr := mdutil.GetString(md.Metadata(), "probe"); addr != "" {
			return addr, true
		}
		// 多跳链路的探测地址列表
		if probes := mdutil.GetStrings(md.Metadata(), "probes"); len(probes) > 0 {
			return strings.Join(probes, ","), true
		}
	}
	return "", false
}

// latencyMsOf 返回候选对象的总延迟
func latencyMsOf(v any) (int64, bool) {
	if latencyProvider == nil {
		return 0, false
	}
	addr, ok := probeAddrOf(v)
	if !ok {
		return 0, false
	}
	// 多地址(逗号分隔): 求和, 任一不可达则整体不可用
	if strings.Contains(addr, ",") {
		var total int64
		for _, a := range strings.Split(addr, ",") {
			ms, ok := latencyProvider.GetLatency(a)
			if !ok {
				return 0, false
			}
			total += ms
		}
		return total, true
	}
	return latencyProvider.GetLatency(addr)
}

type latencyStrategy[T any] struct {
	fallback selector.Strategy[T]
}

// LatencyStrategy 按最佳延迟选择节点/链路, 无可测数据时回退为轮询
func LatencyStrategy[T any]() selector.Strategy[T] {
	return &latencyStrategy[T]{
		fallback: RoundRobinStrategy[T](),
	}
}

func (s *latencyStrategy[T]) Apply(ctx context.Context, vs ...T) (v T) {
	if len(vs) == 0 {
		return
	}

	bestIdx := -1
	var bestMs int64
	for i := range vs {
		ms, ok := latencyMsOf(any(vs[i]))
		if !ok {
			continue
		}
		if bestIdx < 0 || ms < bestMs {
			bestIdx = i
			bestMs = ms
		}
	}
	if bestIdx >= 0 {
		return vs[bestIdx]
	}
	return s.fallback.Apply(ctx, vs...)
}
