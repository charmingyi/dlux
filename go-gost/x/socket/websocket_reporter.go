...
func (w *WebSocketReporter) connect() error {
	...
	var cfg LocalConfig
	if b, err := os.ReadFile("agent.json"); err == nil {
		json.Unmarshal(b, &cfg)
	}

	// 使用最新的配置重新构建 URL
	currentURL := "ws://" + w.addr + "/relay/ws?type=1&secret=" + w.secret + "&version=" + w.version +
...
func (w *WebSocketReporter) routeCommand(cmd CommandMessage) {
	...
	fmt.Println("receive command: ", string(jsonBytes))
	var err error
	...
	// 链路延迟探测配置
	case "UpdateProbes":
		err = w.handleUpdateProbes(cmd.Data)
...
// updateLocalConfigJSON 将 http/tls/socks 写入工作目录下的 agent.json
...
func StartWebSocketReporterWithConfig(addr string, secret string, http int, tls int, socks int, version string) *WebSocketReporter {
	...
	// 构建初始 WebSocket URL
	fullURL := "ws://" + addr + "/relay/ws?type=1&secret=" + secret + "&version=" + version + "&http=" + strconv.Itoa(http) + "&tls=" + strconv.Itoa(tls) + "&socks=" + strconv.Itoa(socks)
...
// handleUpdateProbes 更新链路延迟探测目标
func (w *WebSocketReporter) handleUpdateProbes(data interface{}) error {
	jsonData, err := json.Marshal(data)
	if err != nil {
		return fmt.Errorf("序列化探测配置失败: %v", err)
	}

	var req UpdateProbesRequest
	if err := json.Unmarshal(jsonData, &req); err != nil {
		return fmt.Errorf("解析探测配置失败: %v", err)
	}
...
func (w *WebSocketReporter) handleWgRemove(data interface{}) error {
	jsonData, err := json.Marshal(data)
	if err != nil {
		return fmt.Errorf("序列化移除请求失败: %v", err)
	}

	var req WgRemoveRequest
	if err := json.Unmarshal(jsonData, &req); err != nil {
...
func (w *WebSocketReporter) SendProbes(results []ProbeResult) {
	if len(results) == 0 {
		return
	}
	msg := CommandMessage{
...
func (w *WebSocketReporter) sendRaw(jsonData []byte) {
	w.connMutex.Lock()
	defer w.connMutex.Unlock()
	...
	if w.aesCrypto != nil {
		...
		messageData, _ = json.Marshal(encryptedMessage)
	} else {
		messageData = jsonData
	}
