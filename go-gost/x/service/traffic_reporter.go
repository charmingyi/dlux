...
func SetHTTPReportURL(addr string, secret string) {
	httpReportURL = "http://" + addr + "/relay/traffic?secret=" + secret
	configReportURL = "http://" + addr + "/relay/state?secret=" + secret
...
func sendTrafficReport(ctx context.Context, reportItems TrafficReportItem) (bool, error) {
	...
	req.Header.Set("User-Agent", "Mozilla/5.0 (compatible; NetworkingService/2.0)")
